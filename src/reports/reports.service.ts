...
export class ReportsService {
  async getReports(userId: string, month?: string) {
    let year: number;
    let monthNum: number;
    ...
    for (let i = 0; i <= monthNum; i++) {
      ...
      const monthBillsList = allBills.filter((b) => {
        const dateStr = toDateString(b.due_date);
        return dateStr && dateStr >= sd && dateStr <= ed;
      });
      const monthExpensesList = allExpenses.filter((x) => {
        const dateStr = toDateString(x.date);
        return dateStr && dateStr >= sd && dateStr <= ed;
      });
    ...
    for (let y = currentYear; y >= currentYear - 5; y--) {
      ...
      const yearBills = allBills.filter((b) => {
        const dateStr = toDateString(b.due_date);
        return dateStr && dateStr >= yearStart && dateStr <= yearEnd;
      });
      const yearExpenses = allExpenses.filter((x) => {
        const dateStr = toDateString(x.date);
        return dateStr && dateStr >= yearStart && dateStr <= yearEnd;
      });
